...
export default function Index({ filters, categories, budgets }) {
  ...
  // Mapa por categoria_id -> budget item (se existir)
  ...
  function openCreateOrEdit(categoryId) {
    ...
    setData({
      ...
      year: y,
      month: String(Number(mo)), // backend espera month 1-12
  ...
  function submit(e) {
    ...
    // Se já existe budget para categoria (no mês), fazemos update, senão store (updateOrCreate no backend já cobre)
    // Mas para manter REST “bonito”: se veio id, usa PUT; se não, usa POST.
    ...
    <AuthenticatedLayout
      header={
        <div className="flex items-center justify-between gap-3">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Metas por categoria</h2>
            <p className="text-sm text-gray-500">Defina um teto de gastos mensal e acompanhe o consumo</p>
          </div>

          <Link
            href={route('transactions.index', { month })}
            className="hidden rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 sm:inline-flex"
          >
            Ver lançamentos →
          </Link>
        </div>
        ...
        <div className="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
          ...
          <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-gray-200 sm:p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-3 text-sm text-gray-600">
                <span className="font-semibold text-gray-700">Mês</span>
                <input
                  type="month"
                ...
                />
                <span className="hidden rounded-full bg-gray-50 px-3 py-1 text-xs font-semibold text-gray-700 ring-1 ring-gray-200 sm:inline-flex">
                  {monthLabel}
                </span>
              </div>
              ...
              <Link
                href={route('transactions.index', { month })}
                className="text-sm font-semibold text-emerald-700 hover:text-emerald-800 hover:underline sm:hidden"
              >
                Ver lançamentos →
              </Link>
            </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {(categories || []).map((c) => {
              ...
              const status = b?.status || 'none'; // none | ok | warning | exceeded

              const tone = getBudgetTone(status);
              const pct = clamp(Number(b?.percent || 0), 0, 999);

              return (
                <div key={c.id} className="relative rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-200">
                  {/* Top-right actions */}
                  ...
                  <div className="pr-24">
                    ...
                    {b ? (
                      <div className="mt-2 text-sm text-gray-600">
                        <span className="font-semibold text-gray-900">{formatBRL(b.spent)}</span> de{' '}
                        <span className="font-semibold text-gray-900">{formatBRL(b.amount)}</span>
                      </div>
                    ) : (
                      <div className="mt-2 text-sm text-gray-500">Sem meta definida para este mês.</div>
                    )}
                  </div>
                  ...
                  <div className="mt-4">
                    ...
                    {b && (
                      <div className="mt-3 flex items-center justify-between text-xs text-gray-500">
                        <span>
                          Restante:{' '}
                          <span className="font-semibold text-gray-700">{formatBRL(b.remaining)}</span>
                        </span>
                        {status === 'exceeded' && (
            ...
            {(!categories || categories.length === 0) && (
              <div className="col-span-full rounded-2xl bg-white p-6 text-sm text-gray-500 shadow-sm ring-1 ring-gray-200">
                Você ainda não tem categorias de <b>despesa</b>. Crie uma categoria para definir metas.
                <div className="mt-3">
                  <Link
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div>
                ...
                />
                {errors.year && <div className="mt-1 text-sm text-rose-600">{errors.year}</div>}
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700">Mês</label>
                <select
                  className="mt-1 w-full rounded-lg border-gray-300 text-sm focus:border-emerald-500 focus:ring-emerald-500"
                  value={data.month}
                ...
                {errors.month && <div className="mt-1 text-sm text-rose-600">{errors.month}</div>}
              </div>
            </div>

            <div>
              <label className="block text-sm font-semibold text-gray-700">Teto da meta</label>
              <div className="mt-1">
                <MoneyInput
                  value={data.amount}
...
function Modal({ title, children, onClose }) {
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg rounded-2xl bg-white p-5 shadow-xl ring-1 ring-gray-200">
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <div className="text-lg font-semibold text-gray-900">{title}</div>
            <div className="text-xs text-gray-500">Defina um teto de gastos para o mês selecionado</div>
          </div>
